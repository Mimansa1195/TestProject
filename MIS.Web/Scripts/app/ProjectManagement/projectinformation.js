...
function fetchRoleforCurrentUser() {
    ...
    calltoAjax(misApiUrl.fetchRoleforCurrentUser, "POST", jsonObject,
        function (result) {
...
function listTreeRoots() {
    ...
    if (_isAdminMode) {
        ...
        calltoAjax(misApiUrl.listActiveVerticals, "POST", jsonObject,
            function (result) {
        ...
        calltoAjax(misApiUrl.listRootProjects, "POST", jsonObject,
            function (result) {
                $("#ProjectsTree").dynatree("getRoot").removeChildren();
                if (result == null) {
                    return;
                ...
                for (i = 0; i < result.length; i++) {
                    $("#ProjectsTree").dynatree("getRoot").addChild({ title: result[i].Name, isFolder: true, key: result[i].ProjectId, verticalId: result[i].VerticalId, isLazy: true, expand: false });
                }
            });
    }
}
...
function ListActiveProjectIndustryType(industryTypeId) {
    ...
    calltoAjax(misApiUrl.listActiveProjectIndustryType, "POST", jsonObject,
        function (result) {
...
function ListActiveProjectStatus(statusId) {
    ...
    calltoAjax(misApiUrl.listActiveProjectStatus, "POST", jsonObject,
        function (result) {
            if (result != null && result.length > 0) {
                for (var r = 0; r < result.length; r++) {
...
function fetchSelectedProjectInfo(projectId) {
    ...
    calltoAjax(misApiUrl.fetchSelectedProjectInfo, "POST", jsonObject,
        function (result) {
            if (result.length > 0) {
                ...
                var pOwner = false;
                //var pMember = false;
                if (result[0].Role === "Project Owner" || misSession.roleid === "1") {
                    pOwner = true;
                }
...
function listTeamMembersforSelectedProject(projectId, isActive) {
    ...
    calltoAjax(misApiUrl.listTeamMembersforSelectedProject, "POST", jsonObject,
        function (result) {
            if (result.length != 0) {
...
function saveProject(name, description, industryTypeId, startDate, endDate, statusId) {
    ...
    calltoAjax(misApiUrl.saveProject, "POST", jsonObject,
        function (result) {
            if (result === false) {
...
function updateProject(name, description, industryTypeId, startDate, endDate, statusId) {
    ...
    calltoAjax(misApiUrl.updateProject, "POST", jsonObject,
        function (result) {
            if (result === false) {
...
function listProjects(node) {
    ...
    calltoAjax(misApiUrl.listProjects, "POST", jsonObject,
        function (result) {
            if (result == null) {
            ...
            for (var i = 0; i < result.length; i++) {   
                node.addChild({ title: result[i].Name, isFolder: true, key: result[i].ProjectId, isLazy: true, expand: false });
            }
        });
}
_addMemberRole = '';
...
function listAllUsersAvailableForProject(projectId) {
    ...
    calltoAjax(misApiUrl.listAllUsersAvailableForProject, "POST", jsonObject,
        function (result) {
            if (result == null) {
                return;
            }
...
function listAllUsersAssignedToProject(projectId, role) {
    ...
    calltoAjax(misApiUrl.listAllUsersAssignedToProject, "POST", jsonObject,
        function (result) {
            if (result == null) {
                return;
            }
...
function assignTeamMember(projectId, employeeAbrhs) {
    var jsonObject = {
        ...
        'role': _addMemberRole.replace('s', ''),
        'userAbrhs': misSession.userabrhs
    }
    calltoAjax(misApiUrl.assignTeamMember, "POST", jsonObject,
        function (result) {
...
function unAssignTeamMember(projectId, employeeAbrhs) {
    ...
    calltoAjax(misApiUrl.unAssignTeamMember, "POST", jsonObject,
        function (result) {
            fetchSelectedProjectInfo(_parentProjectId);
...
function fetchProjectInfo() {
    ...
    calltoAjax(misApiUrl.fetchPimcoProjectInfo, "POST", jsonObject,
        function (result) {
            if (result != null) {
                $('#txtProjectName').val(result.Name);
                $('#txtProjectDescription').val(result.Description);
                $('#txtProjectStartDate input').val(result.ShowStartDate);
                $('#txtProjectEndDate input').val(result.ShowEndDate);
                ListActiveProjectIndustryType(result.IndustryTypeId);
                ListActiveProjectStatus(result.StatusId);
                return;
            }
        });
}
